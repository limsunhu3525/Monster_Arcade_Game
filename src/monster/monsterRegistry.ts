import type { MonsterDefinition } from './monsterDefinition';
import { getDefaultMonsterSprite } from './monsterSpriteAssets';

const monsterDefinitions: MonsterDefinition[] = [
  {
    id: 'fire-archetype',
    displayName: '불',
    selectionName: '불',
    selectionDescription: '강한 랜덤 가속과 폭발 넉백으로 한 번에 판을 뒤집는 고위험·고폭발 특성.',
    element: 'FIRE',
    spriteUrl: getDefaultMonsterSprite('FIRE'),
    stats: { massMultiplier: 1, frictionMultiplier: 1, restitutionMultiplier: 1, statusDurationMultiplier: 1 },
    skills: ['FIRE_ACCELERATION', 'FIRE_EXPLOSION'],
    tags: ['high-risk', 'burst', 'chaos'],
  },
  {
    id: 'water-archetype',
    displayName: '물',
    selectionName: '물',
    selectionDescription: '주변 상대를 느리게 만들며 흐름을 통제하는 안정적인 광역 제어 특성.',
    element: 'WATER',
    spriteUrl: getDefaultMonsterSprite('WATER'),
    stats: { massMultiplier: 1, frictionMultiplier: 1, restitutionMultiplier: 0.9, statusDurationMultiplier: 1 },
    skills: ['WATER_ACCELERATION', 'WATER_EXPLOSION'],
    tags: ['control', 'slow'],
  },
  {
    id: 'wind-archetype',
    displayName: '바람',
    selectionName: '바람',
    selectionDescription: '넓은 범위의 돌풍으로 많은 상대의 위치를 한꺼번에 흔드는 광역 교란 특성.',
    element: 'WIND',
    spriteUrl: getDefaultMonsterSprite('WIND'),
    stats: { massMultiplier: 0.9, frictionMultiplier: 1, restitutionMultiplier: 1, statusDurationMultiplier: 1 },
    skills: ['WIND_ACCELERATION', 'WIND_EXPLOSION'],
    tags: ['wide-area', 'displacement'],
  },
  {
    id: 'magic-archetype',
    displayName: '마법',
    selectionName: '마법',
    selectionDescription: '주변 몬스터를 예측하기 어려운 방향으로 흩뜨리는 극단적인 혼돈·변수 생성 특성.',
    element: 'MAGIC',
    spriteUrl: getDefaultMonsterSprite('MAGIC'),
    stats: { massMultiplier: 0.95, frictionMultiplier: 1, restitutionMultiplier: 1, statusDurationMultiplier: 0.9 },
    skills: ['GHOST_SHIFT', 'SPACE_DISTORTION'],
    tags: ['chaos', 'random-displacement'],
  },
  {
    id: 'stone-archetype',
    displayName: '돌',
    selectionName: '돌',
    selectionDescription: '무거운 몸으로 충돌에 버티고 상대를 밀어내는 중량·몸싸움 중심 특성.',
    element: 'STONE',
    spriteUrl: getDefaultMonsterSprite('STONE'),
    stats: { massMultiplier: 1.25, frictionMultiplier: 1, restitutionMultiplier: 0.8, statusDurationMultiplier: 1 },
    skills: ['STONE_HEAVY_MODE', 'STONE_FRAGMENT_EXPLOSION'],
    tags: ['heavy', 'collision'],
  },
  {
    id: 'lightning-archetype',
    displayName: '번개',
    selectionName: '번개',
    selectionDescription: '가장 강한 순간 가속과 짧은 정지 효과로 결정적인 순간을 끊는 폭발적 제압 특성.',
    element: 'LIGHTNING',
    spriteUrl: getDefaultMonsterSprite('LIGHTNING'),
    stats: { massMultiplier: 1, frictionMultiplier: 1, restitutionMultiplier: 1, statusDurationMultiplier: 1 },
    skills: ['LIGHTNING_ACCELERATION', 'LIGHTNING_EXPLOSION'],
    tags: ['speed', 'stun'],
  },
  {
    id: 'earth-archetype',
    displayName: '흙',
    selectionName: '흙',
    selectionDescription: '번개 기절 공격을 막아내고 경사면을 타면 추가 가속을 얻는 방어·지형 활용 특성.',
    element: 'EARTH',
    spriteUrl: getDefaultMonsterSprite('EARTH'),
    stats: { massMultiplier: 1.05, frictionMultiplier: 1.05, restitutionMultiplier: 0.8, statusDurationMultiplier: 1 },
    skills: ['EARTH_WALL_RUN', 'EARTH_QUAKE'],
    tags: ['stable', 'terrain', 'lightning-defense'],
  },
  {
    id: 'ice-archetype',
    displayName: '얼음',
    selectionName: '얼음',
    selectionDescription: '낮은 마찰로 속도를 오래 유지하고 경사면에서 더 강하게 가속하는 고속 관성 특성.',
    element: 'ICE',
    spriteUrl: getDefaultMonsterSprite('ICE'),
    stats: { massMultiplier: 1, frictionMultiplier: 0.55, restitutionMultiplier: 1, statusDurationMultiplier: 1 },
    skills: ['ICE_GLIDE', 'ICE_EXPLOSION'],
    tags: ['slippery', 'momentum', 'slope-speed'],
  },
  {
    id: 'dragon-archetype',
    displayName: '드래곤',
    selectionName: '드래곤',
    selectionDescription: '발동 횟수는 적지만 한 번의 가속과 전방 포효가 매우 강한 고성능·저빈도 특성.',
    element: 'DRAGON',
    spriteUrl: getDefaultMonsterSprite('DRAGON'),
    stats: { massMultiplier: 1.1, frictionMultiplier: 1, restitutionMultiplier: 1, statusDurationMultiplier: 0.75 },
    skills: ['DRAGON_WING_BURST', 'DRAGON_ROAR'],
    tags: ['powerful', 'low-frequency'],
  },
  {
    id: 'beast-archetype',
    displayName: '야수',
    selectionName: '야수',
    selectionDescription: '멈추면 본능적으로 탈출하고 가까운 상대에게 돌진하는 민첩·추적형 특성.',
    element: 'BEAST',
    spriteUrl: getDefaultMonsterSprite('BEAST'),
    stats: { massMultiplier: 0.9, frictionMultiplier: 1, restitutionMultiplier: 1, statusDurationMultiplier: 0.9 },
    skills: ['BEAST_LEAP', 'BEAST_CHARGE'],
    tags: ['agile', 'collision'],
  },
];

const registry = new Map<string, MonsterDefinition>(monsterDefinitions.map((definition) => [definition.id, definition]));

export const getMonsterDefinition = (monsterId: string) => registry.get(monsterId);
export const getAllMonsterDefinitions = () => [...registry.values()];
export const getRegisteredMonsterCount = () => registry.size;
